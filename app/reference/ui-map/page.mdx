...
<img src="/screenshots/agents-anatomy.png" alt="Anatomy" style={{ maxWidth: "100%", border: "1px solid #2a2a2a", borderRadius: "8px", margin: "8px 0" }} />

Models the [Anthropic 2026 Agent](/concepts/mental-model) mockup.

| Region | What it does |
|---|---|
| **Identity tile** (top-left) | Color-hashed icon + name + workspace breadcrumb. Multi-agent dropdown when project has >1 manifest |
| **Subagents pill** (top-right) | Click → drawer listing every tool with usage stats |
| **Skills rail** (left vertical) | Color tiles per skill. Click → Skills tab. `+` tile → Skills tab to add. **Drop target** for skill drags |
...
| **Status bar** (bottom) | Pulsing live dot during runs · "Ready · N runs" otherwise · connector count · orange `+` Discover button → Visual Tool Builder |
