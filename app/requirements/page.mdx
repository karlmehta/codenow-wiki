...
Publishing your built agent to [Agent21](https://agent21.ai) (one-click monetize) requires an `AGENT21_CLIENT_SECRET`.

> **Agent21 is in private beta (invite-only).** You can't self-serve credentials yet.
> Request an invite + client credentials from the CodeNow team; they're issued
> together. Once you have the secret:
>
> 1. Add it to `~/.codenow/.env.local`:
>    ```
>    AGENT21_CLIENT_SECRET=...
>    ```
> 2. Relaunch CodeNow, then click **Connect Agent21** on the Publish tab and approve the OAuth prompt.

If you're not publishing to a marketplace yet, **skip this entirely** — agent scaffolding, running, testing, Live Replay, and every other pipeline step work without an Agent21 account. Publish is the only step that needs it.
